...
class TestVariableScopeClass(_ScopeTestCase):
    ...
    def test_update_scope(self):
        ...
        with be.VariableScope(reuse=False, name=scopes[0]):
            self._check_scope(scopes[0])
            with be.VariableScope(reuse=False, name=scopes[1]):
                self._check_scope(scopes[1])
                with be.VariableScope(reuse=False, name=scopes[2]):
                    self._check_scope(scopes[2])

    def test_update_reuse_flag(self):
        """VariableScope updates current reuse flag"""
        reuse = False
        with be.VariableScope(reuse=reuse, name=self.get_scope()):
            self._check_reuse(reuse)
            reuse = False
            with be.VariableScope(reuse=reuse, name='scope'):
                self._check_reuse(reuse)
            reuse = True
            with be.VariableScope(reuse=reuse, name='scope'):
                self._check_reuse(reuse)
    ...
    def test_reusing_VS_object_correctly_reset_scope(self):
        ...
        with be.VariableScope(reuse=reuse, name=scope) as vs:
            pass

        with be.VariableScope(True, 'scope_to_be_ignored'):
            with vs:
    ...
    def test_VariableScope_correctly_closes_scope(self):
        ...
        for scope in scopes:
            ...
            with be.VariableScope(False, scope):
                pass
    ...
    def test_reuse_variables_enables_flag(self):
        ...
        with be.VariableScope(reuse=False, name=base_scope):
            with be.VariableScope(reuse=False, name='top') as vs:
                vs.reuse_variables()
    ...
    def test_flag_reset_after_reuse_variables(self):
        ...
        base_scope = self.get_scope('base')
        reuse = False
        with be.VariableScope(reuse=reuse, name=base_scope):
            with be.VariableScope(reuse=False, name='top1') as vs:
                vs.reuse_variables()
            self._check_reuse(reuse)

        reuse = True
        with be.VariableScope(reuse=reuse, name=base_scope):
            with be.VariableScope(reuse=False, name='top2') as vs:
                vs.reuse_variables()
            self._check_reuse(reuse)
...
class TestVariableScopeFuncs(_ScopeTestCase):
    ...
    def test_variable_scope(self):
        ...
        with be.variable_scope(scopes[0]):
            self._check_scope(scopes[0])
            with be.variable_scope(scopes[1]):
                self._check_scope('/'.join(scopes[:2]))
                with be.variable_scope(scopes[2]):
    ...
    def test_get_variable_scope(self):
        ...
        scope, reuse = self.get_scope(), False
        with be.variable_scope(scope, reuse=reuse):
    ...
    def test_get_variable_scope_stack(self):
        ...
        with be.variable_scope(scopes[0], reuse=reuses[0]):
            with be.variable_scope(scopes[1], reuse=reuses[1]):
                vs = be.get_variable_scope()
...
class TestGetVariable(_ScopeTestCase):
    ...
    def test_get_variable_reuse_variable(self):
        ...
        var1 = be.get_variable(scope, shape=[3, 1])
        be.scope._set_flag(True)
        var2 = be.get_variable(scope)
        self.assertIs(
    ...
    def test_get_variable_raises_when_reuseing_non_existent_variable(self):
        ...
        be.scope._set_flag(True)
        try:
            be.get_variable('non_existing_variable_name')
        except ValueError:
            pass
        else:
            self.fail(
    ...
    def test_get_variable_raises_when_creating_already_existing_variable(self):
        ...
        scope = self.get_scope()
        be.get_variable(scope, shape=[3, 1])
        try:
            be.get_variable(scope)
        except ValueError:
            pass
        else:
            self.fail(
